...
public class SysMenuController extends BaseController {
	...
	/**
	 * 用户当前用户的菜单和权限信息【导航栏的时候用】
	 * 方法中Principal principal表示注入当前用户的信息，getName就可以获取当当前用户的用户名了。
	 * sysUserService.getUserAuthorityInfo方法我们之前已经说过了，就在我们登录完成或者身份认证时候需要返回用户权限时候编写的。
	 * 然后通过StringUtils.tokenizeToStringArray把字符串通过逗号分开组成数组形式。
	 * @param principal
	 * @return
	 */
	@GetMapping("/nav")
	...
	public Result save(@Validated @RequestBody SysMenu sysMenu) {
		...
		try {
			...
			throw new BusinessException(BusinessExceptionEnum.SYSMENU_CREATE_ERRPR);
		}
